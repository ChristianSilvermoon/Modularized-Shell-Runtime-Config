	...
	case $1 in
		...
		"-?"|"--help")
			...
			local extrahelp=$(
				...
				printf "      %-17s %s\n" "-f [bookmark]" "Print or navigate using \$CD_BKM Associative Array"
				...
				printf "\n    Note: Super CD automatically adds directories to BASH's Directory Stack\n    as a form of history for quick navigation!\n"
			)

			if [ "$(command -v lolcat)" ]; then
				echo "$extrahelp" | lolcat
			else
				echo "$extrahelp"
			fi
